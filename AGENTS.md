...
- Top rule (2026-09-03, Tony's ask): the rink sits ON THE TOP EDGE of the frame. `.ed-stagewrap` is `align-items: flex-start` with 2px of top padding, and `fit()` pulls the stage up by the editor's 170-unit label strip. A centred stage left a dead band above the ice that reads as a broken Notion block.
- Toolbar (redesigned 2026-09-03): a vertical rail on the RIGHT (`#dnRail` = the editor's `.tb` over a `.dn-foot`), 30px buttons, key hints hidden.
  - MINIMAL BY DEFAULT. Only `CORE_TOOLS` (select, skate, pass, the three players, puck) show; everything else gets `.dn-x` and is folded away until More (`#dnMore`, class `dn-more`, remembered in `cthdn.more`). The tagging re-runs on a MutationObserver because `paintTools()` rewrites the bar's innerHTML on every tool change.
  - A SHORTCUT MUST NEVER ARM AN INVISIBLE TOOL: if the armed button carries `.dn-x`, More opens itself.
  - The four colour presets are MOVED OUT of the bar into a 2x2 block in the foot. Four more rows in the tool column was what pushed the core tools into a scroll at embed heights.
  - Duplicate and the key live in the foot, NOT floating over the ice; the publish dot is the small light under them. `.dn-cluster` is now the static viewer's (and the un-embedded Back button's) only.
  - THE RAIL SIZES ITSELF IN JS (`reflow()`). It wraps into a second or third column rather than scrolling or clipping, because a column-wrapping flex box reports only its SINGLE-column width to its parent - left alone the rail stays 46px and silently hides column two. Measure, never predict: the padding, borders and the block the buttons occupy are all read from the DOM (the player swatches carry side margins, so a column is wider than a button). Below three columns' worth of height it falls back to one scrolling column, and the fallback is cleared before each re-probe or the rail never widens again. Every trigger measures twice - a ResizeObserver fires before the layout has settled.
  - AFTER PLACING OR DRAWING, THE TOOL GOES BACK TO SELECT. The shared editor deliberately keeps a tool armed (2026-08-30); the embed wants the opposite, because the next move is almost always adjusting what was just put down. It is done by watching `#edEls` grow across a press and dispatching the editor's own Escape, so no editor state is reached into; open popups and a live text input are skipped.
  - KEYS BELONG TO THE DIAGRAM, NOT THE NOTION PAGE. The editor listens on the iframe's window, and a cross-origin iframe only gets keys while it holds focus - otherwise every letter goes to Notion and edits the page behind the embed. `#dnRoot` is focusable and takes focus on any pointer down (toolbar clicks included); `.dn-focus` is the ring that shows the diagram currently owns the keyboard.
  - Tool popups (`.pmenu`) are re-anchored to the left of the rail by a MutationObserver. The whole rail still collapses for presenting (`#dnTb`, class `dn-tbhide`, remembered in `cthdn.tb`).
